...
Each user is limited to **10 concurrent requests per endpoint**. This means you can have up to 10 in-flight requests to the same endpoint at the same time. Requests to different endpoints are counted separately.

For example, you can simultaneously make:
- 10 requests to `/v1/reddit/posts`
- 10 requests to `/v1/twitter/posts`
- 10 requests to `/v1/linkedin/posts`
...
**Queue your requests** - If you need to make many requests, implement a queue that limits concurrency to 10 per endpoint.
